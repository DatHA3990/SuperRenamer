...
def rename():
        ...
        files = [i.split('\\')[1] for i in [os.path.join("files",f) for root,dirs,files in os.walk(directoryName) for f in files]]
        ...
        if len(name) == len(files):
                ...
                for i in range(len(files)):
                        ...
                        os.rename(directoryName + '\\' + files[i],
                                  directoryName+'\\' + ("%s - "%(str(num)) if isNumbered.get() else "") + name[i].replace('\t','_') + '.' + files[i].split('.')[1])
...
directoryButton.grid(row=3, column=0, sticky=stick)

#create out rename button
renameButton = tk.Button(window, text="Rename", command = rename)
renameButton.grid(row=3, column=1, sticky=stick)
